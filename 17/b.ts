...
class Piece {
  ...
  setBlocks(): void {
    ...
    for(const [x, y] of blocks) {
      board[x][y] = true;
      if(y+cutoff-100 > highest) highest = y+cutoff-100;
    }
  }
}
...
let type = 0;
let jetIndex = 0;
while(type < 1000000000000) {
  const piece = new Piece(3, highest+4+100-cutoff, type++%5 as Types);
  while(true) {
  ...
  if(highest > cutoff+100) { 
    for(let x = 0; x < 7; x++) {
      board[x] = board[x].slice(100);
    }
    cutoff += 100;
  }
}
...
function hashBoard() {
  let hash = "";
  for(let y = highest; y >= highest-100; y--) {
    for(let x = 0; x < 7; x++) {
      hash += board[x][y-cutoff+100] ? "#" : ".";
    }
  }
  return hash;
}

console.log(height+cycleHeight*(1000000000000-61400)/cycle+1);